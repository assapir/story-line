...
describe(`StoryService`, () => {
    ...
    describe(`with data in the database`, () => {
        ...
        describe(`addLineToStory`, () => {
            it(`should throw if no id was given`, async () => {
                try {
                    const line = new Line();
                    line.text = faker.random.words(10);
                    await service.addLineToStory(``, line);
                    failIfReached();
                } catch (error) {
                    expect(error).to.be.instanceOf(BadRequestException);
                    expect(error.message).to.equal(`missing id parameter`);
            ...
            it(`should throw if no line was given`, async () => {
                try {
                    await service.addLineToStory(stories[0].id, new Line());
                    failIfReached();
                } catch (error) {
                    expect(error).to.be.instanceOf(BadRequestException);
                    expect(error.message).to.equal(`missing line parameter or it does not have text`);
                }
            });
            ...
            it(`should throw if no such story found`, async () => {
                try {
                    ...
                    line.text = faker.random.words(10);
                    await service.addLineToStory(`not an id`, line);
                    failIfReached();
                } catch (error) {
                    expect(error).to.be.instanceOf(NotFoundException);
                    expect(error.message).to.equal(`Unable to find story with id 'not an id'`);
            ...
            it(`should throw if the line already belongs to a story`, async () => {
                try {
                    await service.addLineToStory(stories[1].id, lines[0]);
                    failIfReached();
                } catch (error) {
                    expect(error).to.be.instanceOf(NotAllowedException);
                    expect(error.message).to.equal(`can't change line story`);
                }
            });

            it(`should return the story with all it's lines`, async () => {
                const text = faker.lorem.sentence(12);
                const newLine = new Line();
                newLine.id = faker.random.uuid();
                newLine.text = text;

                const result = await service.addLineToStory(stories[0].id, newLine);
                expect(result.lines).to.have.lengthOf(3);
                expect(result.lines[2].text).to.be.equal(text);

                const dbResult = await service.getStory(stories[0].id);
                expect(dbResult.lines[2].storyId).to.be.equal(stories[0].id);
            });
        });
    });
});
